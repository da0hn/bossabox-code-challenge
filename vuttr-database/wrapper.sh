...
log_info "USER: $NEO4J_USER PASSWORD: $NEO4J_PASS"

log_info "Waiting until neo4j stats at :7474 ..."
wget --quiet --tries=10 --waitretry=10 -O /dev/null http://localhost:7474

HAS_DATA=$(echo $(cypher-shell -a bolt://localhost:7687 -u $NEO4J_USER -p $NEO4J_PASS  --format plain "MATCH (n) RETURN count(n) as COUNT") | sed 's/[^0-9]*//g')

log_info "Already has data $(echo ${HAS_DATA}) nodes"

if [[ $HAS_DATA -eq 0 && -d "/cyphers" ]]; then
  log_info "Deleting all relations"
  cypher-shell -a bolt://localhost:7687 -u $NEO4J_USER -p $NEO4J_PASS --format plain "MATCH (n) DETACH DELETE n"
  ...
  for cipherFile in /cyphers/*.cql; do
    ...
    cypher-shell -a bolt://localhost:7687 -u $NEO4J_USER -p $NEO4J_PASS "${contents}"
  done
  log_info "Finished loading all cyphers from '/cyphers'"

  TOTAL_CHANGES=$(cypher-shell -a bolt://localhost:7687 -u $NEO4J_USER -p $NEO4J_PASS --format plain "MATCH (n) RETURN count(n) AS count")
